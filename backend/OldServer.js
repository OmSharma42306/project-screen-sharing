...
const handleStartScreenShare = async (ws, data) => {
    ...
    try {
        ...
        const transport = await createTransport(peerId);
        console.log(`Transport Created for teacher side with peerId = ${peerId} ..`,transport)
        //console.log("Transport Type:", transport.constructor.name);
        console.log("Before peers dictonary : ",peers[peerId]);
        peers[peerId].transports.push(transport);
        console.log("After pushing transports peers dictonary : ",peers[peerId]);
